	...
	function fetchOptions ( $result, $field_val, $field_item, $selected = false, $field_disabled = false, $field_style = false ) {
		...
		if ( $result ) {
			...
			while ( $row = mysqli_fetch_object ( $result ) ) {
				...
				if ( $field_disabled &&  intval ($row -> $field_disabled ) ) $disabled_str = ' disabled="disabled"';
				
				if ( $field_style &&  strlen ( $row -> $field_style ) ) $style = ' style="' . $row -> $field_style . '"';				
				
				$res_str .= '<option' . $selected_str . $disabled_str . $style .  ' value="' . $row -> $field_val . '">' . $row -> $field_item . '</option>';
